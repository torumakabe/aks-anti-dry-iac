...
It's hard to keep up with the evolution of Kubernetes. Especially, upgrade strategy is a headeache.
...
For example, there are several ways to achieve DRY with Terraform. Workspace, Module, specifying git tag as source, git branching strategy & flow, etc. All of these are useful ways, but it is difficult to understand, design, operate and maintain for developers early in the IaC and Kubernetes learning curve. In addition, in the rapidly evolving Kubernetes, it's common to want to redesign the code that creates the cluster. So, standardization of Blue/Green by Module often breaks down.

The codes in this repository are sample of implementation of Azure Kubernetes Services Blue/Green bootstrap & maintenance without adopting DRY. Utilizing Terraform, Flux (v2) and GitHub Actions. The Blue and Green codes are not standarized, but the directories are split. In addtion, also has Terraform states for each environment. You can treat AKS clusters as immutable.
...
However, the difference fo codes between Blue and Green must be easy to see. Therefore, this sample have support steps in CI such as posting the result of diff as a comment at the time of Pull Requests.
...
* Admin
  ...
  * Azure Kubernetes Service Cluster Admin Role (Azure role)
    ...
    * If you are an Azure Subscription Owner, separate assignment is not required
...
Pull Requests trigger the following GitHub Actions as CI. These actions post the result as commnents to the PR.
...
Requests are distributed across both clusters and multiple pods, but session is shared by Redis so it counts correctly.
...
Removed the Service IP of Blue without disruption. So, you can destroy Blue cluster.
